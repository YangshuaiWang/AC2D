...
function geom = bgfc_prep_geom(geom, max_dh, blendw, blendfcn, useP2)
...
% (also create a second array of pure volumes rather than effective
%   volumes; this is needed for B-QCF assembly)
geom.volT = zeros(1, geom.nT);
geom.volT1 = zeros(1, geom.nT);
facd = factorial(geom.dDim);
detA = det(geom.A);
for k = 1:geom.nT
    
    % TODO: calculate physical volume of T
    p = geom.X(:, geom.T(1:3, k));
    ...
    % create a second array of "pure" volumes
    geom.volT1(k) = geom.volT(k);
...
if useP2 == 1
    % stores the FEM order in triangles
    geom.nX0 = geom.nX;
    % geom.T1 stores the edge dofs
    geom.T1 = zeros(size(geom.T));
    
    for j = 1:geom.nE
        T = geom.aET(:, j);
        % adding edge dof if the edge is in the continuum region and not on the
        % boundary
        if min(geom.di(geom.E(:,j))) < 0 && min(T) > 0
            geom.X = [geom.X, (geom.X(:, geom.E(1,j))+geom.X(:, geom.E(2,j)))/2];
            geom.nX = geom.nX + 1;
            % geom.numEdgeDofinT(T) = geom.numEdgeDofinT(T) + 1;
            % geom.T1(geom.numEdgeDofinT(T(1)), T(1)) = geom.nX;
            % geom.T1(geom.numEdgeDofinT(T(2)), T(2)) = geom.nX;
            [~, ia] = setdiff(geom.T(:, T(1)), geom.E(:,j));
            geom.T1(ia, T(1)) = geom.nX;
            [~, ia] = setdiff(geom.T(:, T(2)), geom.E(:,j));
            geom.T1(ia, T(2)) = geom.nX;
            geom.XonE(geom.nX) = j;
        end
    end
    
    for k = 1:geom.nT
        if min(geom.di(geom.T(1:3, k))) < 0
            geom.pn(k) = 2;
        end
    end
    % extend geom.T by its edge dofs
    geom.T = [geom.T; geom.T1];
    geom.nX = size(geom.X, 2);
    geom.volX = [geom.volX, zeros(1, geom.nX-geom.nX0)];
    
    geom.useP2 = useP2;
    Neigs = geom.Neigs;
    geom.Neigs = sparse(geom.nX, geom.nX);
    geom.Neigs(1:geom.nX0, 1:geom.nX0) = Neigs;
...
%% test code
function test_bgfc_prep_geom()
figure(1);
N = 12; dH = 2; bw = 2; K = dH + 2*bw; useP2 = 1;
% profile on;
geom = geom_2dtri_hexagon(N, K, 3/2);
geom = geom_analyze(geom);
geom = bgfc_prep_geom(geom, dH, bw, 'min-hessian', useP2);
% profile off;
% profile report;
geom.plot(geom);
...
%    text(geom.X(1,n)+0.1, geom.X(2,n), num2str(geom.volX(n)));
    text(geom.X(1,n), geom.X(2,n), num2str(n));
end

for k = 1:geom.nT
    % get barycentre of element k
    x = 0.333 * sum(geom.X(:, geom.T(1:3, k)), 2);
    % text(x(1), x(2), num2str(geom.volT(k), '%4.2f'));
    % text(x(1), x(2), num2str(geom.pn(k), '%d'));
    str = ['\color{red}', num2str(k)];
    text(x(1), x(2), str);
end

figure(2);
% plot blending function
trisurf(geom.T(1:3,:)', geom.X(1,:)', geom.X(2,:)', geom.volX);
axis([-N, N, -N, N, -2, 3]);
end

...
%% Plots  
mrkrs = {'o-', 's-', '^-', '*-'};
cols = {[0,0,0], [1,0,0], [0,0.7,0], [0,0,1]};
meth(1).actype = 'ATM';
meth(2).actype = 'QCE';
meth(3).actype = 'B-QCE';
meth(4).actype = 'QNL';
...
loglog(dof, 1*(dof/dof(1)).^(-1/2), 'k:', 'Linewidth', lwth);
...
loglog(dof, 0.2*(dof/dof(1)).^(-1), 'k:', 'Linewidth', lwth); 
annotation(currfig,'textbox',...
...
axis([dof(1) / 1.5, dof(2)*1.5, 3e-3, 5e-1]);
% legend('QCE', 'Linear Blending', 'Smooth Blending', 'DoF^{-1/2}', 'Location', 'Southwest');
set(gca, 'XMinortick', 'off', 'YMinorTick', 'off', 'Fontsize', 18);
legend(acmethods, 'Location', 'Northeast');

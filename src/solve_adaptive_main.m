...
function [geom, model, Y] = solve_adaptive_main(geom, model, Y, dirname, sample, fn)
% check input
N = geom.N;
dof = geom.nX;

load([dirname,'/setup.mat'], 'data');
F0 = data.F0;
...
if slflag == 1 && exist('estimate', 'var')
    switch sample
        case 'GD'
%             solve_adaptive_disp(geom, estimate);
            fprintf('Data: %s loaded!\n', str);
        case 'ED'
%             solve_eb_disp(geom, estimate);
            fprintf('Data: %s loaded!\n', str);
        otherwise
            fprintf('Unkown sample : %s\n', sample);
            keyboard;
    end
else
    switch sample
...
%               below! Or above to compute stab structure.
            estimate = solve_adaptive_error(geom, model, Y, fn);
        case 'ED'
%             [estimate, ~] = solve_adaptive_eb(geom, model, Y, dirname, F0);
%             same as above!
            estimate = solve_adaptive_eb(geom, model, Y, fn, F0);
        otherwise
            fprintf('Unkown sample : %s\n', sample);
            keyboard;
    end
